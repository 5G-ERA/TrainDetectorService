...
class WrappedDetectorWorker(TrainDetectorWorker):
    ...
    def __init__(self, output_queue, time_measurements, verbose_processing, **kw_args) -> None:
        ...
        for _ in range(3):
            self.detector.process_image(empty_frame)

    def publish_results(self, data, metadata) -> None:
...
def main():
    parser = argparse.ArgumentParser(description='Example with direct local usage of the train detector.')
    ...
    # init detector
    detector_thread = WrappedDetectorWorker(output_queue, time_measurements, args.verbose, image_queue=image_queue, app=None, name="Detector", daemon=True)
